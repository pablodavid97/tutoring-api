...
router.post('/files-upload', async (req, res) => {
  ...
  fs.writeFileSync(
    semestersPath,
    new Buffer.from(files.semesters.datos, 'binary')
  );
  fs.writeFileSync(deansPath, new Buffer.from(files.deans.datos, 'binary'));
  fs.writeFileSync(
    professorsPath,
    new Buffer.from(files.professors.datos, 'binary')
  );
  fs.writeFileSync(
    studentsPath,
    new Buffer.from(files.students.datos, 'binary')
  );
...
router.post('/add-users', async (req, res) => {
  ...
  await fileParserAdditionalUsers(
    deansPath,
    professorsPath,
    studentsPath,
    adminsPath
...
router.post('/additional-files', async (req, res) => {
  ...
  if (files.careers) {
    ...
    fs.writeFileSync(
      careersPath,
      new Buffer.from(files.careers.datos, 'binary')
    );
  }
...
let fileParser = async (filesPath) => {
  ...
  let roles = [
    { id: DECANO_ROL, name: 'Decano' },
    { id: PROFESOR_ROL, name: 'Profesor' },
    { id: ESTUDIANTE_ROL, name: 'Estudiante' },
    { id: ADMIN_ROL, name: 'Administrador' }
  ];
  let estadosReunion = [
    { id: 1, estado: 'Creada' },
    { id: 2, estado: 'Editada' },
    ...
    { id: 6, estado: 'En Progreso' },
    { id: 7, estado: 'Realizada' },
    { id: 8, estado: 'No Realizada' }
  ];
  let estadosNotificacion = [
    { id: 1, estado: 'Activa' },
    { id: 2, estado: 'Leida' },
    { id: 3, estado: 'Archivada' }
  ];
  let superuser = {
    id: 1,
    correoInstitucional: 'admin@usfq.edu.ec',
    codigo: '',
    hash: '$2a$10$.Mqxo.yyACYbzsHfSR2ih.65jZ0H0z4IR6Hrcor6TWPtoGk77j.nq',
    nombres: 'Admin',
    apellidos: 'User',
    correoPersonal: '',
    telefono: '',
  ...
  await usuarioController.insertUser(
    idCounter,
    superuser,
    superuser.firstTimeLogin
  );
  ...
  for (decano of decanos) {
    ...
    if (userId === -1) {
      ...
      await usuarioController.insertUser(
        idCounter,
        decano,
        DEFAULT_FIRST_TIME_LOGIN_VALUE
      );
  ...
  for (estudiante of estudiantes) {
    ...
    if (userId === -1) {
      ...
      await estudianteController.insertEstudiante(
        idCounter,
        estudiante,
        profesorId,
        carreraId
      ...
      await rolesUsuarioController.insertRolDeUsuario(
        idCounter,
        ESTUDIANTE_ROL
      );
...
let fileParserAdditionalUsers = async (
  deansPath,
  professorsPath,
  studentsPath,
  adminsPath
  ...
  if (deansPath) {
    ...
    for (dean of deans) {
      deanUser = await usuarioViewController.getUserByEmail(
        dean.correoInstitucional
      );
  ...
  if (professorsPath) {
    ...
    for (professor of professors) {
      professorUser = await usuarioViewController.getUserByEmail(
        professor.correoInstitucional
      );
  ...
  if (studentsPath) {
    ...
    for (student of students) {
      studentUser = await usuarioViewController.getUserByEmail(
      ...
      if (studentUser === undefined) {
        ...
        tutor = await usuarioViewController.getUserByEmail(
          student.correoProfesor
        );
  ...
  if (adminsPath) {
    ...
    // Inserta la lista de admins en la BD
    for (admin of admins) {
      adminUser = await usuarioViewController.getUserByEmail(
...
let fileParserAdditionalData = async (careersPath, semestersPath, gpasPath) => {
  ...
  if (gpasPath) {
    ...
    // Inserta los gpas por semestre en la BD
    for (gpa of gpas) {
      let user = await usuarioViewController.getUserByEmail(
        gpa.correoInstitucional
      );
