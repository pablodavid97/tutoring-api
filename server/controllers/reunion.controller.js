...
// CREATE MEETING
reunionController.createMeeting = async (subject, description, date, professorId, studentId, email) => {
  try {
    meeting = await reunion.create({
      ...
      createdBy: email, 
      createdOn: new Date()},
      {fields: ['tema', 'descripcion', 'fecha', 'profesorId', 'estudianteId', 'estadoId', 'createdBy', 'createdOn']})
    return meeting
  } catch (error) {
...
reunionController.deleteMeeting = async (reunionId, userEmail) => {
  try {
    await reunion.update({estadoId: 5, updatedOn: new Date(), updatedBy: userEmail}, {
      where: {
        id: reunionId
      }
    });
